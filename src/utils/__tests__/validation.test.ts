...
describe('validateInputs', () => {
  ...
  const validMembers: Member[] = [
    ...
    },
    {
      ...
      location: { lat: 35.6462, lng: 139.6527 },
      isDriver: false,
    },
  ];
  ...
  it('目的地が未入力の場合エラーになること', () => {
    const destination = { addressInput: '', location: undefined };
    const result = validateInputs(validMembers, destination);
    expect(result.isValid).toBe(false);
    expect(result.errors).toContain('目的地を入力してください。');
  });
  
  it('メンバーが2人未満の場合エラーになること', () => {
    const members = [validMembers[0]];
  ...
  it('車ありメンバーの定員が未設定の場合エラーになること', () => {
    const members = [
      { ...validMembers[0], vehicleCapacity: undefined },
      validMembers[1],
    ];
    const result = validateInputs(members, validDestination);
    expect(result.isValid).toBe(false);
  ...
  it('総定員不足の場合警告が出ること', () => {
    const members = [
      { ...validMembers[0], vehicleCapacity: 1 }, // ドライバーのみ
      validMembers[1],
    ];
    const result = validateInputs(members, validDestination);
    expect(result.isValid).toBe(true);
